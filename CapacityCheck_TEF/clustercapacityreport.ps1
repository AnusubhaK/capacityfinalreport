...
function Get-ClusterCapacityCheck {
    ...
    $ClusterFreeStorageGB = ($VMHost | Get-Datastore | Where-Object {$_.Extensiondata.Summary.MultipleHostAccess -eq $True} | Measure-Object -Property FreeSpaceGB -Sum).Sum
    ...
    ClusterFreeStorageGB = $ClusterFreeStorageGB
...
Get-Cluster | Get-ClusterCapacityCheck | Select-Object Cluster,ClusterFreeCPUCore,ClusterFreeMemoryGB,ClusterFreeStorageGB |
Export-Csv -Path C:\Users\anusubha\Desktop\vcenterclustercapreport.csv -NoTypeInformation -UseCulture